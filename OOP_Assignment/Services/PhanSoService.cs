...
public class PhanSoService : IOOPAssignments
{
    ...
    public void Display()
    {
        ...
        //Tinh toan
        var phanSoCong = PhepCong(phanSo1, phanSo2);
        var phanSoTru = PhepTru(phanSo1, phanSo2);
        var phanSoNhan = PhepNhan(phanSo1, phanSo2);
        var phanSoChia = PhepChia(phanSo1, phanSo2);
    ...
    private PhanSo ChuanHoa(PhanSo tempPhanSo)
    {
        long ucln;
        int check = new();
        if (tempPhanSo.TuSo == 0) return new PhanSo(0, tempPhanSo.MauSo);
        ...
        tempPhanSo.TuSo /= ucln;
        if (check == 1) tempPhanSo.TuSo *= -1;
        tempPhanSo.MauSo /= ucln;
        return tempPhanSo;
    }
}